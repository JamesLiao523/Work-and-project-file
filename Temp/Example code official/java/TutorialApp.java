...
class JavaCallBack extends CCallBack 
{
	public boolean OnDataPoint(CDataPoint oDataPt)
	{
		DecimalFormat f = new DecimalFormat("0.000");		

		System.out.println( "Risk(%) = " + f.format(oDataPt.GetRisk())+
				    " \tReturn(%) = " + f.format(oDataPt.GetReturn()));
		return false;
	}

	public boolean OnMessage(CMessage oMessage)
	{
		System.out.println( "Message: " + oMessage.GetMessage() );
		return false;
	}
}
...
{
	protected Set<String> m_DumpTID; 
	...
	/// Initialize the optimization
	public void Initialize(String tutorialID, String description, boolean setAlpha)
	{
		Initialize(tutorialID, description, dumpWorkspace(tutorialID), setAlpha, false);
	}
	
	/// Initialize the optimization
	public void Initialize(String tutorialID, String description, boolean setAlpha, boolean isTaxAware)
	{
	...
	*/
	public void Tutorial_1d()
	{
		...
		m_Case.InitConstraints().EnableRoundLotting(false); 
	...
	*/
	public void Tutorial_1e()
	{
		...
		m_Case = m_WS.CreateCase("Case 1e", m_InitPf, m_TradeUniverse, portfolioBaseValue);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
	...
	*/
	public void Tutorial_1f()
	{
		...
		// Create a case object, null trade universe
		m_Case = m_WS.CreateCase("Case 1f", m_InitPf, null, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
		...
		System.out.println("Initial portfolio statistics:");
		System.out.println("Return = " + f.format(m_Solver.Evaluate(EEvalType.eRETURN)) );
		double factorRisk = m_Solver.Evaluate(EEvalType.eFACTOR_RISK);
		double specificRisk = m_Solver.Evaluate(EEvalType.eSPECIFIC_RISK);
		System.out.println("Common factor risk = " + f.format(factorRisk));
		System.out.println("Specific risk = " + f.format(specificRisk) );
		System.out.println("Active risk = " + f.format(Math.sqrt(factorRisk*factorRisk + specificRisk*specificRisk)) );
		System.out.println("Short rebate = " + f.format(m_Solver.Evaluate(EEvalType.eSHORT_REBATE)) );
		System.out.println("Information ratio = " + f.format(m_Solver.Evaluate(EEvalType.eINFO_RATIO)) );
		System.out.println(); 

		CPortfolio portfolio = m_Solver.GetPortfolioOutput().GetPortfolio();
		System.out.println("Optimal portfolio statistics:");
		System.out.println("Return = " + f.format(m_Solver.Evaluate(EEvalType.eRETURN, portfolio)) );
		...
		System.out.println("Specific risk = " + f.format(specificRisk) );
		System.out.println("Active risk = " + f.format(Math.sqrt(factorRisk*factorRisk + specificRisk*specificRisk)) );
		System.out.println("Short rebate = " + f.format(m_Solver.Evaluate(EEvalType.eSHORT_REBATE, portfolio)) );
		System.out.println("Information ratio = " + f.format(m_Solver.Evaluate(EEvalType.eINFO_RATIO, portfolio)) );
		System.out.println();
	}
	...
	*/
	public void Tutorial_1g()
	{
		...
		// Create a case object, null trade universe
		m_Case = m_WS.CreateCase("Case 1g", m_InitPf, null, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
		...
		System.out.println("Is type of optimization problem convex: " + (m_Case.IsConvex() ? "Yes" : "No"));
		
		//retrieve paring constraints
		DecimalFormat f = new DecimalFormat("0");
 		System.out.println("max number of assets is: " 
 			+ f.format(paring.GetAssetTradeParingRange(EAssetTradeParingType.eNUM_ASSETS).GetMax()) + "\n");
		...
		if ( output != null ) 
		{
			System.out.println("The output portfolio is " + (output.IsHeuristic() ? "heuristic" : "optimal"));
			CIDSet softBoundSlackIDs = output.GetSoftBoundSlackIDs();
			if (softBoundSlackIDs.GetCount() > 0)
				System.out.println("Soft bound violation found");
		}
	}
	...
	*/
	public void Tutorial_2a()
	{
		...
		m_TradeUniverse.AddAsset("COMP1");
	...
	*/
	public void Tutorial_3a()
	{
		...
		for(int j=0; j<m_Data.m_AssetNum; j++)
		{
			...
			// Set asset penalty
	 		if ( m_Data.m_ID[j].compareTo("USA11I1") == 0 ) {
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 3b ========");
		System.out.println("Factor Range Constraints");
		...
		System.out.println("Initial portfolio exposure to Factor_1A = " + f.format(exposure));
		...
		if (output != null)
		{
			...
			if (slackInfo != null) {
				System.out.println("Optimal portfolio exposure to Factor_1A = " +
									f.format(slackInfo.GetSlackValue()));
	...
	*/
	public void Tutorial_3c2()
	{
		...
		CAttributeSet assetBetaSet = rm.ComputePortAssetBeta(m_TradeUniverse, m_BM2Portfolio);
		...
		System.out.println("Optimal portfolio's beta relative to benchmark in utility = " + f.format(beta));
		double beta2 = rm.ComputePortBeta(output.GetPortfolio(), m_BM2Portfolio);
		System.out.println("Optimal portfolio's beta relative to second benchmark = " + f.format(beta2));
	}
	...
	*/
	public void Tutorial_3h()
	{
		...
		DecimalFormat f = new DecimalFormat("0.0000");
		System.out.println("Total active weight = " + f.format(sumActiveWeight));
	}
	...
	*/
	public void Tutorial_3i()
	{
		...
		DecimalFormat f = new DecimalFormat("0.0000");
		System.out.println("Sum of all weights = " + f.format(sumWeight));
	}
	...
	*/
	public void Tutorial_3j()
	{
		...
		for(int j=0; j<m_Data.m_AssetNum; j++) {
			...
			if (m_Data.m_ID[j].compareTo("CASH")!=0) {
				CConstraintInfo info = linear.SetAssetRange(m_Data.m_ID[j]);
	...
	/**\brief Maximum Number of Assets and estimated utility upper bound
	*
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 4a ========");
		System.out.println("Max # of assets and estimated utility upper bound");
		SetupDumpFile ("4a");
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 5a ========");
		System.out.println("Piecewise Linear Transaction Costs");
		SetupDumpFile ("5a");
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 5b ========");
		System.out.println("Nonlinear Transaction Costs");
		SetupDumpFile ("5b");
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 5c ========");
		System.out.println("Transaction Cost Constraint");
		SetupDumpFile ("5c");
	...
	*/
	public void Tutorial_5e()
	{
		...
		if (status.GetStatusCode() != EStatusCode.eOK) {
			System.out.println("Error loading transaction cost data: " + status.GetMessage());
			System.out.println(status.GetAdditionalInfo());
		}
	...
	*/
	public void Tutorial_7a()
	{
		...
		if ( pfOut != null ) {
			DecimalFormat f = new DecimalFormat("0.0000");
			System.out.println( "Specific Risk(%) = " + f.format(pfOut.GetSpecificRisk()) );
			System.out.println( "Factor Risk(%) = " + f.format(pfOut.GetFactorRisk()) );

			CRiskConstraints riskConstraint = m_Case.InitConstraints().InitRiskConstraints();

			System.out.println();
			System.out.println( "Add a risk constraint: FactorRisk<=12%" );
			...
			System.out.println("Add a risk constraint: Factor_1B-1E<=1.9%");
			System.out.println();
			...
			if (pfOut2 != null)
			{
				System.out.println( "Specific Risk(%) = " + f.format(pfOut2.GetSpecificRisk()) );
				System.out.println( "Factor Risk(%) = " + f.format(pfOut2.GetFactorRisk()) );
			}			
		}
	}
	...
	*/
	public void Tutorial_7b()
	{
		...
		CConstraintInfo info = riskConstraint.AddPLTotalConstraint(true, m_BM2Portfolio);
		...
		if (output != null)
		{
			...
			{
				DecimalFormat f = new DecimalFormat("0.0000");
				System.out.println("Risk Constraint Slack = " + f.format(slackInfo.GetSlackValue()));
				System.out.println();
			}
		}
	}
    ...
    */
    public void Tutorial_7c()
    {
	    ...
	    if ( pfOut != null ) {
		    ...
		    System.out.println("Risk from USA13Y1 & 1TY1 = " + 
                f.format(m_Solver.EvaluateRisk(pfOut.GetPortfolio(), ERiskType.eTOTALRISK, null, aid, null, true, true)));
		    System.out.println("Risk from Factor_7|8|9* = " + 
                f.format(m_Solver.EvaluateRisk(pfOut.GetPortfolio(), ERiskType.eFACTORRISK, null, null, fid, true, true)));
			System.out.println();

			CRiskConstraints riskConstraint = m_Case.InitConstraints().InitRiskConstraints();

            System.out.println("Add a risk constraint(additive def): from USA13Y1 & 1TY1 <=1%");
		    CConstraintInfo info = riskConstraint.AddTotalConstraint( aid, null, true, null, false, false, false, true);
		    info.SetUpperBound(0.01);

		    System.out.println("Add a risk constraint(additive def): from Factor_7|8|9* <=1.9%");
			System.out.println("");
			...
			if (pfOut2 != null)
			{
                ...
                System.out.println("Factor Risk(%) = " + f.format(pfOut2.GetFactorRisk()));
			    System.out.println("Risk from USA13Y1 & 1TY1 = " +  
                    f.format(m_Solver.EvaluateRisk(pfOut2.GetPortfolio(), ERiskType.eTOTALRISK, null, aid, null, true, true)));
			    System.out.println("Risk from Factor_7|8|9* = " +  
                    f.format(m_Solver.EvaluateRisk(pfOut2.GetPortfolio(), ERiskType.eFACTORRISK, null, null, fid, true, true)));
				System.out.println("");

			    CIDSet ids = pfOut2.GetSlackInfoIDs();
				String id=ids.GetFirst();
				for(int i = 0; i < ids.GetCount(); i++, id = ids.GetNext())
				    System.out.println("Risk Constraint Slack of " + id + " = " + f.format(pfOut2.GetSlackInfo(id).GetSlackValue()));
				System.out.println("");
			}
    ...
    public void Tutorial_7d()
    {
        ...
        if (pfOut != null)
        {
            ...
            for (String id = ids.GetFirst(); !id.equals(""); id = ids.GetNext())
                System.out.format("Risk Constraint Slack of %s = %.4f\n", id, pfOut.GetSlackInfo(id).GetSlackValue());
            System.out.println("");
        }
    }
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 8a ========");
		System.out.println("Long-Short Hedge Optimization");
		SetupDumpFile ("8a");
	...
	*/
	public void Tutorial_8b()
	{
		...
		{
	 		CConstraintInfo info = linear.SetAssetRange(m_Data.m_ID[j]);
			if(m_Data.m_ID[j].compareToIgnoreCase("CASH")!=0)
			{
				info.SetLowerBound(-1.0);
				info.SetUpperBound(1.0);
			}
    ...
    */
    ...
    {
        System.out.println("======== Running Tutorial 8c ========");
        System.out.println("Weighted Total Leverage Constraint Optimization");
		SetupDumpFile ("8c");
	...
	*/
	public void Tutorial_10a()
	{
		...
		m_Case = m_WS.CreateCase("Case 10a", m_InitPf, m_TradeUniverse, pfValue, 
						0.0);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
		...
		if (output != null)
		{
			...
			if (taxOut != null)
			{
				...
				System.out.println("Long Term Gain  = " + f.format(taxOut.GetLongTermGain()));
				System.out.println("Long Term Loss  = " + f.format(taxOut.GetLongTermLoss()));
				System.out.println("Long Term Tax   = " + f.format(taxOut.GetLongTermTax()));
				System.out.println("Short Term Gain = " + f.format(taxOut.GetShortTermGain()));
				System.out.println("Short Term Loss = " + f.format(taxOut.GetShortTermLoss()));
				System.out.println("Short Term Tax  = " + f.format(taxOut.GetShortTermTax()));
				System.out.println("Total Tax       = " + f.format(taxOut.GetTotalTax()));
				System.out.println();
	...
	public void Tutorial_10c()
	{
		...
		if ( output != null ) { 
			...
			if (taxOut != null) {
				...
				DecimalFormat f = new DecimalFormat("0.0000");
				System.out.println( "Tax info for group GICS_SECTOR/Information Technology:" );
				System.out.println("Long Term Gain  = " + f.format(lgg) );
				System.out.println("Long Term Loss  = " + f.format(lgl) );
				System.out.println("Short Term Gain = " + f.format(sgg) );
				System.out.println("Short Term Loss = " + f.format(sgl) );
		...
		if (output != null)
		{
			...
			if (taxOut != null)
			{
				...
				for (String assetID = idSet.GetFirst(); !assetID.equals(""); assetID = idSet.GetNext())
				{
					...
					for (String lotID = lotIDs.GetFirst(); !lotID.equals(""); lotID = lotIDs.GetNext())
					{
						double shares = sharesInTaxlot.GetValue(lotID);
    ...
    public void Tutorial_10e()
    {
        ...
        if (output != null) {
            ...
            if (taxOut != null) {
                ...
                System.out.println("Long Term Tax   = " + f.format(ltax));
                System.out.println("Short Term Tax  = " + f.format(stax));
                System.out.println("Loss Benefit    = " + f.format(lb));
                System.out.println("Total Tax       = " + f.format(tax) + "\n");
	...
	public void Tutorial_10f()
	{
		...
		if (output != null) {
			...
			if (taxOut != null) {
				...
				System.out.println("Tax info (Financials):\n");
				System.out.println("Total Gain  = " + f.format(tgg));
				System.out.println("Total Loss  = " + f.format(tgl));
				System.out.println("Total Net   = " + f.format(tgn) + "\n");
				...
				System.out.println("Tax info (Information Technology):\n");
				System.out.println("Total Gain  = " + f.format(tgg));
				System.out.println("Total Loss  = " + f.format(tgl));
				System.out.println("Total Net   = " + f.format(tgn) + "\n");
	...
	public void Tutorial_10g()
	{
		...
		if (output != null) {
			...
			if (taxOut != null) {
				...
				for (String assetID = assetIDs.GetFirst(); !assetID.equals(""); assetID = assetIDs.GetNext()) {
					...
					if (wsDetail != null) {
						for (int i = 0; i < wsDetail.GetCount(); i++) {
							...
							int age = wsDetail.GetAdjustedAge(i);
	...
	*/
	public void Tutorial_11a()
	{
		...
		{        
			//show error message
			String err = oStatus.GetMessage();
			System.out.println("Optimization error: " + err);
		}
		...
		for (int i=0; i<frontierOutput.GetNumDataPoints(); i++)
		{
			...
			System.out.println("Risk(%) = " + f.format(dataPoint.GetRisk())+
							   " \tReturn(%) = " + f.format(dataPoint.GetReturn()));
		}

		System.out.println();

		System.out.println( "Interactive approach..." );
		m_Solver.SetCallBack(new JavaCallBack());
		CStatus oStatus2 = m_Solver.Optimize();
		System.out.println(oStatus2.GetMessage());
		System.out.println();
	}
    ...
    */
    public void Tutorial_11b()
    {
        ...
        DecimalFormat f6 = new DecimalFormat("0.000000");
        ...
        for (int i = 0; i < frontierOutput.GetNumDataPoints(); i++)
        {
            ...
            System.out.println("Utility = " + f6.format(dataPoint.GetUtility())
				+ "\tRisk(%) = " + f3.format(dataPoint.GetRisk())
				+ "\tReturn(%) = " + f3.format(dataPoint.GetReturn()));

            System.out.println("Optimal portfolio exposure to Factor_1A = " + f4.format(dataPoint.GetConstraintSlack()));
        }
        System.out.println();
    }
    ...
    public void Tutorial_11c()
    {
        ...
        for (int i = 0; i < frontierOutput.GetNumDataPoints(); i++)
        {
            ...
            System.out.println("Utility = " + f6.format(dataPoint.GetUtility())
				+ "\tRisk(%) = " + f3.format(dataPoint.GetRisk())
				+ "\tReturn(%) = " + f3.format(dataPoint.GetReturn()));
            System.out.println("Optimal portfolio exposure to Information Technology = "
                + f4.format(dataPoint.GetConstraintSlack()));
        }
        System.out.println();
    }
    ...
    public void Tutorial_11d()
    {
        ...
        if (frontierOutput != null)
        {
            for (int i = 0; i < frontierOutput.GetNumDataPoints(); i++)
            {
                CDataPoint dataPoint = frontierOutput.GetFrontierDataPoint(i);
				System.out.println("Utility = " + f6.format(dataPoint.GetUtility())
					+ "   Total leverage = " + f3.format(dataPoint.GetConstraintSlack()));
            }
        }
        else
            System.out.println("Invalid frontier");
	...
	*/
	public void Tutorial_14a()
	{
		...
		// Create a case object, null trade universe
		m_Case = m_WS.CreateCase("Case 14a", m_InitPf, m_TradeUniverse, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
		...
		if ( output != null ) {
			...
			{
				DecimalFormat f = new DecimalFormat("0.0000");
				System.out.println("Shortfall Beta Con Slack = " + f.format(slackInfo.GetSlackValue()));
				System.out.println();
			}
		}
	}
	...
	*/
	public void Tutorial_15a()
	{
		...
		// Create a case object, null trade universe
		m_Case = m_WS.CreateCase("Case 15a", m_InitPf, null, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
	...
	*/
	public void Tutorial_15b()
	{
		...
		CConstraintInfo info = riskConstraint.AddPLTotalConstraint(false, m_BM2Portfolio);
		info.SetID( "RiskConstraint" );
		info.SetUpperBound(0.1);
	...
	*/
	public void Tutorial_16a()
	{
		...
		// Create a case object, null trade universe
		m_Case = m_WS.CreateCase("Case 16a", m_InitPf, null, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
	...
	*/
	public void Tutorial_16b()
	{
		...
		// Create a case object, null trade universe
		m_Case = m_WS.CreateCase("Case 16b", m_InitPf, null, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
		...
		util.AddCovarianceTerm(0.0075, ECovTermType.eXWFWX, null, attributeSet); 
	...
	*/
	public void Tutorial_17a()
	{
		...
		m_Case = m_WS.CreateCase("Case 17a", m_InitPf, m_TradeUniverse, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));
    ...
    public void Tutorial_18()
    {
		...
		// Create a case object and trade universe
		m_Case = m_WS.CreateCase("Case 18", m_InitPf, m_TradeUniverse, 100000);
		m_Case.SetPrimaryRiskModel(m_WS.GetRiskModel("GEM"));

        CRiskModel riskModel = m_WS.GetRiskModel("GEM");
        CIDSet factorGroupA = m_WS.CreateIDSet();
        ...
        CIDSet factorGroupB = m_WS.CreateIDSet();
    ...
    */
    public void Tutorial_19()
    {
        ...
        if (status != ERiskModelStatus.eSUCCESS) {
            System.out.println("Failed to load risk model data using Models Direct files");
            return;
        }
        m_Case.SetPrimaryRiskModel(rm);
        ...
        if ( pfOut != null ) {
            ...
            if ( slackInfo != null){
                System.out.println("Optimal portfolio exposure to USE4L_SIZE = " + f.format(slackInfo.GetSlackValue()) );
            }
        }
    }
	...
	*/
	public void Tutorial_19b()
	{
		...
		{
			System.out.println(numStatus.GetMessage());
			System.out.println(numStatus.GetAdditionalInfo());
			return;
		} 
	...
	*/
	...
	{
		System.out.println("======== Running Tutorial 20 ========" );
		System.out.println("Minimize Total Risk" );
		SetupDumpFile ("20");
		...
		if (status.GetStatusCode() != EStatusCode.eOK) {
			System.out.println("Error loading asset exposures data: " + status.GetMessage());
			System.out.println(status.GetAdditionalInfo());
		}
	...
	*/
	public void Tutorial_21(){
		System.out.println("======== Running Tutorial 21 ========");
		System.out.println("Retrieve KKT terms of constraint & asset attributions");
		SetupDumpFile ("21");
	...
	*/
	public void Tutorial_23()
	{
		...
		DecimalFormat f = new DecimalFormat("0.0000");	
		System.out.println("Portfolio conentration="
			+f.format(m_Solver.Evaluate(EEvalType.ePORTFOLIO_CONCENTRATION, m_Solver.GetPortfolioOutput().GetPortfolio())));
	}
	...
	*/
	public void Tutorial_25a()
	{
		...
		util.SetAlphaTerm(1.5);
		// Set benchmark, risk aversions for specific (lambdaD) and common factor (lambdaF) to 0.0075
		util.SetPrimaryRiskTerm(m_BM2Portfolio, 0.0075, 0.0075);
		// Set constraints
		CConstraintInfo range = linearConstraint.SetAssetRange("USA13Y1");
		range.SetLowerBound(0.2);

		// Set constraints for all accounts and/or cross-account
		m_WS.SwitchAccount(barraopt.getALL_ACCOUNT());
		...
		if( oStatus.GetStatusCode() == EStatusCode.eOK ) {
			...
			if ( output != null ) {
				...
				System.out.println("Joint Market Impact Buy Cost($) = " + f.format(output.GetJointMarketImpactBuyCost()));
				System.out.println("Joint Market Impact Sell Cost($) = " + f.format(output.GetJointMarketImpactSellCost()));
				System.out.println();
				...
				for (int i=0; i<output.GetNumAccounts(); i++) {
					...
					System.out.println("Account     = " + fi.format(pAccountOutput.GetAccountID()));
	...
	*/
	public void Tutorial_26()
	{
		...
		CConstraints cons = m_Case.InitConstraints();
		CIssuerConstraints issuerCons = cons.InitIssuerConstraints();
	...
	*/
	public void Tutorial_28a()
	{
		...
		if (output != null) {
			DecimalFormat f = new DecimalFormat("0.0000");
			CSlackInfo slackInfo = output.GetSlackInfo(info.GetID());
			System.out.println("Ratio       = " + f.format(slackInfo.GetSlackValue()));
			System.out.println();
		}
	}
	...
	*/
	public void Tutorial_28b()
	{
		...
		if (output != null) {
			DecimalFormat f = new DecimalFormat("0.0000");
			CSlackInfo slackInfo = output.GetSlackInfo(info.GetID());
			System.out.println("Financials / IT = " + f.format(slackInfo.GetSlackValue()));
			slackInfo = output.GetSlackInfo(info2.GetID());
			System.out.println("Minerals / IT   = " + f.format(slackInfo.GetSlackValue()));
			System.out.println();
		}
	}
	...
	public void Tutorial_29()
	{
		...
		boolean is_positive_semidefinite = Q_mat.IsPositiveSemidefinite();
	...
	protected boolean dumpWorkspace(String tid){
    ...
    */
    protected void PrintLowerAndUpperBounds(CLinearConstraints cons)
    {
		DecimalFormat f = new DecimalFormat("0.00");
        ...
        for (int i = 0; i < ids.GetCount(); i++, cid = ids.GetNext())
        {
            ...
            System.out.println("constraint ID: " + info.GetID());
            System.out.println("lower bound: " + f.format(info.GetLowerBound()) 
								+ ", upper bound: " + f.format(info.GetUpperBound()) + "\n");
        }
    }
    ...
    */
    protected void PrintLowerAndUpperBounds(CHedgeConstraints cons)
    {
		DecimalFormat f = new DecimalFormat("0.00");
    ...
    */
    protected void PrintParingConstraints(CParingConstraints paring)
    {
		DecimalFormat f = new DecimalFormat("0");
		...
		if( paring.ExistsAssetTradeParingType(EAssetTradeParingType.eNUM_ASSETS) )
			System.out.println("Minimum number of assets is: " 
				+ f.format(paring.GetAssetTradeParingRange(EAssetTradeParingType.eNUM_ASSETS).GetMin()));
		if( paring.ExistsAssetTradeParingType(EAssetTradeParingType.eNUM_TRADES) )
			System.out.println("Maximum number of trades is: " 
				+ f.format(paring.GetAssetTradeParingRange(EAssetTradeParingType.eNUM_TRADES).GetMax()));

		ELevelParingType lps [] = { 
			ELevelParingType.eMIN_HOLDING_LONG,
        ...
        for (int i=0; i<6; i++)
	        if( paring.ExistsLevelParingType(lps[i]) )
				    System.out.println( msg[i] + ff.format(paring.GetThreshold(lps[i])));
        System.out.println("\n");
    }
    ...
    */
    protected void PrintConstraintPriority(CConstraintHierarchy hier)
    {	
		ECategory cate[] = {
			ECategory.eLINEAR,
			ECategory.eFACTOR,
        ...
        for(int i=0; i<12; i++) 
            if( hier.ExistsCategoryPriority(cate[i]) ){
				...
				if(order==ERelaxOrder.eFIRST)
					System.out.println("The category priority for " + cate_string[i]+ " is the first");
				else if(order==ERelaxOrder.eSECOND)
					System.out.println("The category priority for " + cate_string[i]+ " is the second");
				else if (order==ERelaxOrder.eLAST)
					System.out.println("The category priority for " + cate_string[i]+ " is the last");
			};
