...
# 引入策略類別
from xqctrader.Strategies.CAPMLTSBStrategy import CAPMLTSBStrategy as Strategy# 統計套利策略
# 引入Redis來連接自建的即時報價
...
def runBacktest(symbol_list, data, stock_to_future, point_multiplier, margin_multiplier, exchanges, type_dict, days):
    ...
    datahandler = DataHandler(eventmanager, data, stock_to_future.values())

    # Initialize strategy
    # 策略的參數設定
    strategy = Strategy(datahandler, eventmanager, point_multiplier, type_dict, start_end=[start_time, end_time], exit_time=133000, dataFreq=DataFreq.Intraday, tradeStatus=TradeStatus.BacktestIntraday, stock_to_future=stock_to_future)
    
    # Initialize Portfolio
    account_balance = {'Capital':10000000.0, 'Margin':10000000.0} # 模擬交易之初始資金
    portfolio = Portfolio(datahandler, eventmanager, point_multiplier, start_date, account_balance, type_dict=type_dict, margin_multiplier=margin_multiplier)
    
    # Initialize Profitloss
    stop_loss = 15000 / sum(account_balance.values())
    ...
    statistic = TearsheetStatistics(portfolio, datahandler, eventmanager, title=f'simulate BackTest trading of {str(strategy)} with {len(symbol_list)}', 
                                    periods=periods, aggregateToDaily=True if days > 1 else False, tradeStatus=TradeStatus.BacktestIntraday)
...
if __name__ == "__main__":
    ...
    runBacktest(symbol_list, data, stock_to_future, point_multiplier, margin_multiplier, exchanges, type_dict, days)
